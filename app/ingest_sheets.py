...
def main():
    ...
    for sheet_name, mapping in SHEETS_CONFIG.items():         # Перебираем пары «имя листа» → «правила»
        ...
        for row in tqdm(rows, desc=f"Читаем лист '{sheet_name}'"):  # Прогресс по строкам листа
            # Формируем payload из всех нужных колонок FAQ с использованием маппинга
            payload = {
                "id": row.get(mapping["id_col"]),
                "question": row.get(mapping["question_col"]),
                "answer": row.get(mapping["answer_col"]),
                "category": row.get(mapping["category_col"]),
                "tags": row.get(mapping["tags_col"]),
                "source": row.get(mapping["source_col"]),
                "image_url": row.get(mapping["image_url_col"]),
                "last_updated": row.get(mapping["last_updated_col"])
            }
            question = str(payload.get("question") or "")
            answer = str(payload.get("answer") or "")
            if not answer and not question:
                continue
            # Вектор строим по объединению question + answer
            text_for_embedding = (question + " " + answer).strip()
            ...
            if not payload.get("id"):
                payload["id"] = pid  # Сохраняем сгенерированный id в payload для консистентности
